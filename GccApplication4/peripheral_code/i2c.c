...
#define TWI0_BUSSTATE ((TWI0.MSTATUS & TWI_BUSSTATE_gm) >> TWI_BUSSTATE_gp)
...
void bmg_read_routine(void)
{
    ...
    {
        TWI0.MCTRLB = (1 << TWI_ACKACT_bp) | (0x3 << TWI_MCMD0_bp);  // send NACK (to discontinue reading) followed by STOP condition
    ...
    else if(_state == READSTATE_PRE_READ_VALUE)
    {
        *_data_ptr = TWI0.MDATA;
        ...
        {
            TWI0.MCTRLB = (0 << TWI_ACKACT_bp) | (0x2 << TWI_MCMD0_bp);  // send ACK (to continue reading) followed by a byte read operation
        }
    }
}
...
void setup_i2c(void)
{
    ...
    /* twi */
    TWI0.CTRLA = (0 << TWI_SDASETUP_bp) |           // SDA setup time is 4 clock cycles
                 (0x3 << TWI_SDAHOLD_gp) |          // hold SDA after SCL 500ns
                 (0 << TWI_FMPEN_bp);               // fast mode plus disabled

    TWI0.MBAUD = 161;                               // target 10kHz f_SCL , T_rise ~= 300ns --> 2*BAUD = (3333333/10000) - 10 - 1

    TWI0.MCTRLA = (1 << TWI_RIEN_bp) |              // enable master read interrupt
                  (1 << TWI_WIEN_bp);               // enable master write interrupt

    TWI0.MSTATUS = (0x1 << TWI_BUSSTATE_gp);        // force from unknown state (on startup) to idle
